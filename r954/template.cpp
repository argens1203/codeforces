...
// For finding and removing the smallest first
...
void solve(){
    ...
    dp[0][0] = 0;
    for (int t_idx = 1; t_idx <= k; ++t_idx){
        for (int mask = 0; mask < (1 << R); ++mask){
            dp[t_idx][mask] = dp[t_idx - 1][mask];
            for (int r = 1; r <= R; ++r){
                dp[t_idx][mask] = max(dp[t_idx][mask],
                    dp[t_idx - 1][mask ^ (1 << (r - 1))] + precal[t_idx][r - 1]
                );
            }
        }
    }
